...
class CmService
{
    ...
    {
        $filename = $slug . '.zip';
        $url = Config::get()->byKey('url') . $filename;

        $this->rep->download($url, Config::get()->byKey('modulePath') . $slug);

        $data = ['version' => $this->getVersion($slug), 'status' => 'inactive', 'type' => 'module'];

        $this->mod->save($slug, $data);

        $url_loc = Config::get()->byKey('module' . 'Path') . $filename;

        if ($this->unpack($url_loc, Config::get()->byKey('module' . 'Path'), $slug)) {
